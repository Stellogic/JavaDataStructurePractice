...
**为什么这会失败？**
问题就出在当 `i` 和 `j` 相遇时，相遇点的元素 `nums[i]` 可能是大于 `key` 的。这样，在执行swap(nums,left,i)时，可能把一个较大的数换到最左边。
...
**问题分析**：
因为是 `i` 指针先移动并找到了一个“大数”（`1`）而停下，而 `j` 指针因为 `i` 已经到达了它的位置而没有机会再去寻找“小数”，导致 `i` 和 `j` 最终相遇在了一个大于基准数的位置上。最后一步交换，就把这个“大数”换到了数组的最左边，破坏了分区的正确性。

---

使用填坑法更好理解

1.  **挖坑**：选定一个基准数（通常是第一个元素），把它从数组里“拿”出来，存在一个临时变量里。现在，数组的第一个位置就是一个“坑”。
2.  **填坑**：
    *   坑在**左边**时，就从**右边**找一个小数来填。
    *   填完后，右边就出现了一个新坑。
    *   坑在**右边**时，就从**左边**找一个大数来填。
    *   填完后，左边就出现了一个新坑。
3.  **循环**：不断重复第2步，直到左右两个指针相遇。
4.  **归位**：相遇的位置就是最后的坑，把手上一直拿着的那个基准数放进去。

```java
// 填坑法实现
int partition_hole(int[] nums, int left, int right) {
    int i = left, j = right;
    int pivot = nums[left]; // 把基准值存起来，形成一个坑
    while (i < j) {
        // 1. j从右向左找小数，填到左边的坑(i)里
        while (i < j && nums[j] >= pivot) {
            j--;
        }
        if (i < j) {
            nums[i] = nums[j]; // 把小数填到左边，j的位置成了新坑
            i++;
        }
        // 2. i从左向右找大数，填到右边的坑(j)里
        while (i < j && nums[i] <= pivot) {
            i++;
        }
        if (i < j) {
            nums[j] = nums[i]; // 把大数填到右边，i的位置成了新坑
            j--;
        }
    }
    // 循环结束，i和j相遇，这就是最后的坑，把基准值放进去
    nums[i] = pivot;
    return i;
}
```
...
### 下面是最好理解的Lomuto分区。统一按照这个理解记忆

1.  **选择基准 (Pivot)**：选择数组的最后一个元素作为基准值。
2.  **初始化指针**：
    *   `i` 指针（慢指针）初始化在数组起始位置的前一个位置 (`low - 1`)。它用来指向“小于等于基准”区域的最后一个元素位置。
    *   `j` 指针（快指针）从数组的起始位置 `low` 开始遍历到基准元素的前一个位置。
3.  **遍历与交换**：
    *   `j` 指针向右移动，逐个检查元素。
    *   如果 `array[j]` 的值小于或等于基准值，就意味着它应该属于左边的区域。
    *   此时，将 `i` 指针向右移动一位，然后交换 `array[i]` 和 `array[j]` 的值。这个操作实际上就是将小于等于基准的元素 `array[j]` 放入了左侧区域。
4.  **放置基准**：
    *   当 `j` 指针遍历完所有元素后，所有小于等于基准的元素都已经被移动到了数组的左边（`low` 到 `i` 的位置）。
    *   此时 `i+1` 的位置就是基准元素应该在的最终位置。将基准元素（`array[high]`）与 `array[i+1]` 进行交换。
5.  **返回基准索引**：返回 `i+1`，作为下一轮递归的分割点。
...
public class QuickSortLomuto {
    ...
    public static void quickSort(int[] arr, int low, int high) {
        // 只有当 low < high 时，才有排序的必要
        if (low < high) {
            // 调用 partition 方法，找到基准元素的正确位置 pi
     ...
     * @param arr   待分区数组
     * @param low   分区的起始索引
     * @param high  分区的结束索引 (也是基准元素所在的位置)
     * @return      基准元素分区后的最终索引
     */
    public static int partition(int[] arr, int low, int high) {
        ...
        // 2. i 是 "小于等于" 基准区域的边界指针
        // 初始化在 low 的前一个位置
        int i = (low - 1);

        // 3. j 是遍历指针，从 low 遍历到 high - 1
...
void siftDown(int[] nums,int n,int i){
    //这个sift Down实质上就是选取当前根节点和左右子节点，把其中最大的和根节点交换位置
...
下面我们来证明log(n-1) + ……+log2的渐进上限是O(nlogn)
1. 显然和s<=nlogn
2. 之后我们把s分两半，log(2)到log(n/2),和log(n/2+1)到log(n-1)，后一半大于(n/2)log(n/2),因此s>=nlogn
